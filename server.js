...
function getYouTubeEmbedUrl(videoId, mode = "video") {
...
app.get("/api/youtube/embed", (req, res) => {
  const youtubeUrl = clean(req.query.url);
  const mode = clean(req.query.mode) || "video";
  ...
  res.json({
    ...
    video_id: videoId,
    mode,
    embed_url: getYouTubeEmbedUrl(videoId, mode)
  });
});
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const artist = clean(req.body.artist);
      const album = clean(req.body.album);
      const category = clean(req.body.category) || "All Songs";
      const language = clean(req.body.language);
      const genre = clean(req.body.genre);

      const audioUrl = nullable(req.body.audio_url);
      const youtubeUrl = nullable(req.body.youtube_url);
      const coverUrl = nullable(req.body.cover_url);
      const lyrics = clean(req.body.lyrics);
      ...
      if (req.file) {
        ...
        finalAudioUrl = uploaded.secure_url;
      }
...
async function startServer() {
  try {
    ...
    app.listen(PORT, "0.0.0.0", () => {
      ...
      console.log(`Cloudinary: ${cloudinaryReady ? "configured" : "NOT CONFIGURED"}`);
      console.log("YouTube embedded playback: ENABLED");
      console.log("Premium liquid player: ENABLED");
      console.log("====================================");
    });
  } catch (error) {
